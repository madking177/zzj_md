...
首先获取dex起点和尺寸，然后在合适的时机（明文阶段）

dexfileparser（解析dex文件），openmemory参数包含dex开始位置和大小

fart的脱壳核心就是包含dex地址起点与尺寸的逻辑段

java中脱壳，可以通过类关联到其所在的dex，getdex，getbytes两个函数就可以完成dexdump，但是如果
